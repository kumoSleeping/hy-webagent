...
> 当前行为：普通会话的完整页面 URL（`/chat/<piSessionId>`）就是公开的
> 只读链接。未登录访客打开它时会进入预览模式，无法发送消息或执行写入操作。
...
完整会话 UUID 是普通聊天页 URL 的一部分。因此把地址栏中的
`/chat/<piSessionId>` 发给别人后，对方无需登录即可阅读完整会话（包括后续
流式更新）。服务端只接受完整 UUID，拒绝部分 ID、路径穿越和无效格式；访客
...
它不再是普通 `/chat/:id` URL 的访问门槛；完整 UUID 页面 URL 已按上节的
产品策略公开为只读。除普通页面 URL 外，访客也仍可通过下列两种方式打开旧链接：

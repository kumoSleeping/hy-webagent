...
> 背景：`01-security-fixes.md` 的 S1 把未认证的访客只读模式收紧为
> 「只允许通过 bot 频道发布的会话」，这堵住了漏洞，但也让**普通会话无法再分享**。
> 本文档描述补回该能力的显式授权机制。
...
访客连接**不携带任何凭证**。因此服务端面对一个 `piSessionId` 时，
唯一能问的问题是：「有没有证据表明**所有者本人**愿意公开它？」

改动前没有这样的证据，代码只能退化成「你说得出 id 就给你看」——
而 id 既可以从公开的 bot 面板拿到，又因为当时是子串匹配，连猜都不用猜。

所以补回分享能力的正确方式不是放宽校验，而是**制造出这个证据**：
一个由所有者主动签发、可撤销、会过期的令牌。

现在访客授权有且只有两条依据：

1. 会话通过**已启用的 bot 频道**发布（原有能力，不变）；
2. 调用方持有所有者为**该会话**签发的、未撤销未过期的**分享令牌**（本次新增）。

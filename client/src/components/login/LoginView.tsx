...
export function LoginView() {
  ...
  return (
    <div className="flex h-full items-center justify-center bg-[var(--pi-bg)] px-4">
      <div className="pi-panel-card relative w-full max-w-md p-8 pt-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" autoComplete="on">
          ...
          <input
            ...
            placeholder="sk-hyw-..."
            autoFocus
            className="w-full border border-[var(--pi-line)] bg-[var(--pi-panel)] px-3.5 py-2.5 text-base text-[var(--pi-text)] outline-none transition focus:border-[var(--pi-line)]"
          />
          {error && <p className="text-xs text-[var(--pi-danger)] font-mono">{error}</p>}
          <button
            type="submit"
            disabled={isLoading || !apiKey.trim()}
            className="flex h-10 w-full items-center justify-center bg-[var(--pi-text)] text-sm font-bold uppercase tracking-wider text-[var(--pi-panel)] transition-all hover:bg-[var(--pi-text-strong)] disabled:cursor-not-allowed disabled:opacity-40"
          >

...
export function buildSecuritySystemPrompt(): string {
  return [
    ...
    "   designated workspace directory (including `projects/` and that user's `.pi/`).",
    ...
    "   top, and htop require a one-time per-session user confirmation echo (see platform",
    "   SYSTEM.md). Before confirming, explain that only user-owned workspace processes",
    "   may be targeted — never system services or other users' resources.",
    "",
    "6. **Tools & Dependencies**: Users may install helper packages only under their",
    "   workspace `projects/` directory. Never install to system paths or global env.",
    "",
